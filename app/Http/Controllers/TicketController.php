...
class TicketController extends Controller
{
        ...
        $tickets = Ticket::where(function ($query) {
            ...
            ->orWhere(function ($query) {
                $query->where('t_status', 'OPENED')
                    ->where('t_assignedTo', 'Not Assigned');
            })
            ->get();
    ...
    public function sendEmail($tickets)
    {
        $allStaff = Reporter::whereNotIn('u_role', ['Client'])->get();
        $ticketss = count($tickets);
        ...
        foreach ($allStaff as $staffs) {
            ...
            $message = '<html lang="en">
                    ...
                    <div class="card">
                        ...
                        <h2 class="mb-0">NEW TICKETS</h2>
                        <p>Hi <strong>' . $emailSendTo->u_name . '</strong><br>,
                <br> This email is to inform you that there are ' . $ticketss . 'tickets that has been unopened for a while now. Please check them out.<br><hr>    
                    <p>Ticket IDs: ' . implode(', ', $tickets->t_ID) . '</p>
            ...
            $mail->Subject = "Unresolved Tickets";
            $mail->Body = $message;
